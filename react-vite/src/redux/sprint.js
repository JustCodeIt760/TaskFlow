...
export const thunkSetSprint = (projectId, sprintId) => async (dispatch, getState) => {
    dispatch(setLoading(true));
    ...
    try {
        const response = await csrfFetch(`/projects/${projectId}/sprints/${sprintId}`);
        if (!response.ok) {
...
export const thunkUpdateSprint = (projectId, sprintData) => async (dispatch) => {
    dispatch(setLoading(true));
    try {
        const response = await csrfFetch(`/projects/${projectId}/sprints/${sprintData.id}`, {
            method: 'PUT',
            body: JSON.stringify(sprintData),
            headers: {
                'Content-Type': 'application/json',
...
const sprintReducer = (state = initialState, action) => {
    const handlers = {
        ...
        [REMOVE_SPRINT]: (state, action) => {
            const newState = { ...state };
            const { [action.payload]: removedSprint, ...remainingSprints } = newState.allSprints;
